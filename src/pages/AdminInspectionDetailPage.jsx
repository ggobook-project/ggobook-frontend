...
export default function AdminInspectionDetailPage() {
  ...
  const handleApprove = async () => {
    if (!window.confirm("이 작품과 회차를 최종 승인하시겠습니까? (AI 요약이 함께 진행됩니다.)")) return;
  ...
  const handleReject = async () => {
    ...
    if (rejectReason === "기타 (직접 작성)" && !customRejectReason.trim()) { alert("상세 반려 사유를 작성해주세요."); return; }
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className={styles.mainContent}>
        <section className={styles.sectionCard}>
          ...
          <div className={styles.contentInfo}>
            <div className={styles.thumbnailArea}>
              {content.thumbnailUrl ? <img src={content.thumbnailUrl} alt="작품 썸네일" /> : <div className={styles.noThumbnail}>썸네일 없음</div>}
            </div>
            <div className={styles.textInfo}>
              <div className={styles.infoRow}><span className={styles.label}>작품 제목:</span><span className={styles.value}>{content.title}</span></div>
              <div className={styles.infoRow}><span className={styles.label}>작가 정보:</span><span className={styles.value}>{content.author?.nickname ? `${content.author.nickname} (ID: ${content.author.id})` : (content.author?.id || "미상")}</span></div>
              <div className={styles.infoRow}><span className={styles.label}>장르 / 타입:</span><span className={styles.value}>{content.genre} / {content.type}</span></div>
              <div className={styles.infoRow}><span className={styles.label}>작품 요약:</span><p className={styles.description}>{content.summary}</p></div>
            </div>
          </div>
        </section>
        <section className={styles.sectionCard}>
          ...
          <div className={styles.manuscriptArea}>
            {content.type === "웹소설" || content.type === "NOVEL" ? (
              <div className={styles.novelText}>{data.contentText || data.novel?.contentText || "등록된 웹소설 원고가 없습니다."}</div>
            ) : (
              <div className={styles.webtoonImages}>
                {(() => {
                  const imageList = data.comicToons || data.images || data.webtoonImages || [];
                  if (imageList.length === 0) return <div style={{padding: "20px", color: "#666"}}>등록된 웹툰 이미지가 없습니다.</div>;
                  return imageList.map((img, idx) => (
                    <img key={img.image_id || idx} src={img.imageUrl} alt={`웹툰 컷 ${idx + 1}`} style={{ width: "100%", marginBottom: "10px", display: "block", borderRadius: "8px" }} />
                  ));
                })()}
              </div>
            )}
        ...
        <div className={styles.actionBar}>
          <div className={styles.actionInfo}>검토 결과에 따라 승인 또는 반려를 선택해주세요.</div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            ...
            {rejectReason === "기타 (직접 작성)" && (
              <textarea value={customRejectReason} onChange={(e) => setCustomRejectReason(e.target.value)} placeholder="상세 사유를 입력해주세요..." className={styles.modalTextarea} disabled={isProcessing} />
            )}
