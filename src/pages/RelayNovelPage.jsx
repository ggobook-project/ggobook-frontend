    ...
    <div style={{ background: c.bg, minHeight: "calc(100vh - 60px)" }}>
      <div style={{ background: `linear-gradient(135deg, ${c.primarySoft} 0%, ${c.bgWhite} 100%)`, padding: "32px 40px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          ...
          <button onClick={() => navigate("/relay/register")} style={{ padding: "10px 20px", background: c.primary, border: "none", borderRadius: theme.radius.md, color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>+ 새 릴레이</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 40px" }}>
        ...
        {relays.map(r => (
          <div key={r.id} onClick={() => navigate(`/relay/${r.id}`)} style={{
