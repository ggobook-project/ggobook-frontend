...
export default function AuthorContentDetailPage() {
  ...
  const goToViewer = (epId) => {
    ...
    navigate(
      isWebtoon
        ? `/webtoon/viewer/${epId}?contentId=${contentId}`
        : `/novel/viewer/${epId}?contentId=${contentId}`,
    );
  };
    ...
    <div className={styles.pageWrapper}>
      <div className={styles.header}>
        ...
        <div className={styles.headerMeta}>
          {content.type} · {content.genre} · 총 {episodes.length}화
        </div>
      </div>
      ...
      <div className={styles.content}>
        <div className={styles.listHeader}>
          <span className={styles.listTitle}>
            회차 목록{" "}
            <span className={styles.listCount}>{episodes.length}</span>
          </span>
          <button
            className={styles.registerBtn}
            onClick={() =>
              navigate(`/author/contents/${contentId}/episode/register`)
            }
          >
            회차 등록
          </button>
          ...
          episodes.map((ep) => (
            ...
            >
              <div className={styles.episodeLeft}>
                ...
                <div>
                  <div className={styles.episodeTitle}>
                    {ep.episodeNumber}화 : {ep.episodeTitle || ep.title}
                  </div>
              ...
              <div className={styles.episodeRight}>
                <span
                  className={`${styles.epStatusBadge} ${styles[STATUS_STYLE[ep.status]] || styles.statusReview}`}
                >
                ...
                {ep.status !== "PENDING" && ep.status !== "BLINDED" && (
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      navigate(
                        `/author/contents/${contentId}/episode/${ep.episodeId}/edit`,
                      );
                    }}
                  >
                    수정
