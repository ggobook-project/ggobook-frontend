...
export default function RelayNovelRegisterPage() {
  ...
  const handleSubmit = async () => {
    if (useAdminTopic && !selectedTopic) {
      await showAlert("주제를 선택해주세요.");
      return;
    }
    // 글자 수 검증
    if (startText.length < MIN_CHARS || startText.length > MAX_CHARS) {
      await showAlert(
        `시작 내용은 ${MIN_CHARS}자 이상 ${MAX_CHARS}자 이하로 작성해주세요.`,
      );
      return;
    }
    if (!title.trim() || !startText.trim()) {
      ...
      <div className={styles.content}>
        <div className={styles.formCard}>
          ...
          <div className={styles.formGroup}>
            ...
            />
            <div
              className={styles.charCountWrap}
            ...
            >
              ...
              {startText.length < MIN_CHARS && startText.length > 0 && (
                <span style={{ color: "#E53935", marginLeft: "10px" }}>
                  ({MIN_CHARS - startText.length}자 더 필요)
                </span>
              )}
            </div>
          </div>
