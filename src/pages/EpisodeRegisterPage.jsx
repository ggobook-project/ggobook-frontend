...
export default function EpisodeRegisterPage() {
    ...
    const initData = async () => {
      ...
      try {
        const [contentRes, lastEpRes] = await Promise.all([
          ...
          api.get(`/api/author/contents/${contentId}/episodes`, {
            params: { size: 1, sort: "episodeNumber,desc" },
          }),
        ]);
        ...
        if (isEdit) {
          ...
          if (ep.novel) {
            setIsNovel(true);
            setNovelText(ep.novel.contentText || "");
          } else if (ep.comicToons) {
            setIsNovel(false);
            setWebtoonImages(
              ep.comicToons
          ...
          setEpisodeNumber(
            lastEpList.length > 0 ? lastEpList[0].episodeNumber + 1 : 1,
          );

          if (lastEpList.length > 0) {
            const lastDate = new Date(
              lastEpList[0].scheduledAt || lastEpList[0].createdAt,
            );
  ...
  const handleRemoveImage = (idToRemove) =>
    setWebtoonImages((prev) => prev.filter((img) => img.id !== idToRemove));
  ...
  const handleSubmit = async () => {
    ...
    if (isNovel && !novelText.trim())
      return showAlert("원고 내용은 필수입니다.");
    if (!isNovel && webtoonImages.length === 0)
      return showAlert("웹툰 이미지를 1장 이상 업로드해주세요.");
    ...
    if (isNovel) {
      formData.append(
        "novel",
        new Blob([JSON.stringify({ contentText: novelText })], {
      ...
      webtoonImages.forEach((img) => {
        if (img.type === "OLD") {
          formData.append("imageOrder", img.url);
        } else {
          formData.append("imageOrder", "NEW_FILE");
          formData.append("episodeFiles", img.file);
        }
      });
    }
          ...
          <div className={styles.formGroup}>
            ...
            <div className={styles.formGroup}>
              ...
              <DatePicker
                ...
                onChange={(date) => {
                  if (date) setScheduledAt(getLocalIsoString(date));
                }} // 🌟 안전한 로컬 타임 적용
              ...
              {!isFixedDate && !isEdit && (
                ...
                >
                  * 지정하신 연재 요일에 해당하는 날짜만 선택할 수 있습니다.
                  (최소 3일 이후)
                </div>
              )}
            </div>
