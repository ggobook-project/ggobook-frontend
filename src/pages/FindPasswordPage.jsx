    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: c.bg }}>
      <div style={{ width: "100%", maxWidth: 400, padding: "0 24px" }}>
        <div style={{ background: c.bgWhite, borderRadius: theme.radius.lg, border: `1px solid ${c.border}`, padding: "40px 36px", boxShadow: "0 4px 24px rgba(33,150,243,0.1)" }}>
          ...
          <button style={{ width: "100%", padding: 12, background: c.primary, border: "none", borderRadius: theme.radius.md, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", marginBottom: 14 }}>재설정 이메일 발송</button>
          <div style={{ display: "flex", justifyContent: "center", gap: 20 }}>
            <span onClick={() => navigate("/login")} style={{ fontSize: 12, color: c.textMuted, cursor: "pointer" }}>로그인</span>
            <span onClick={() => navigate("/find-id")} style={{ fontSize: 12, color: c.textMuted, cursor: "pointer" }}>아이디 찾기</span>
          </div>
        </div>
      </div>
    </div>
