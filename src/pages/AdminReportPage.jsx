  ...
  return (
    <div className={styles.pageWrapper}>
      <div className={styles.header}>
        ...
        <div className={styles.headerSubtitle}>사용자들이 접수한 신고 내역을 검토하고 조치하세요</div>
      </div>
      ...
      <div className={styles.content}>
        ...
        {loading ? (
          ...
          <div style={{textAlign: "center", padding: "50px", color: "#666"}}>대기 중인 신고가 없습니다. 🎉</div>
        ) : (
            ...
            <div key={r.reportId} className={styles.reportCard}>
              <div className={styles.reportTop}>
                <div className={styles.reportLeft}>
                  ...
                  <span className={styles.reportReasonBadge}>{REASON_MAP[r.reportReason] || r.reportReason}</span>
                </div>
                <span className={styles.reportDate}>{r.createdAt?.split('T')[0]}</span>
              </div>
              <div className={styles.reportInfo}>
                <div><strong>대상 유저:</strong> {r.reportedUser?.nickname} ({r.reportedUser?.userId})</div>
                <div><strong>신고자:</strong> {r.reporter?.nickname}</div>
                <div className={styles.targetLink}>타겟 ID: {r.targetId} (클릭 시 해당 콘텐츠로 이동하도록 구현 가능)</div>
              </div>

              {/* 🌟 3가지 버튼 세팅 */}
              <div className={styles.reportActions} style={{ display: "flex", gap: "8px", justifyContent: "flex-end", marginTop: "12px" }}>
                <button 
                  className={styles.rejectBtn} 
                  style={{ padding: "6px 12px", border: "1px solid #ccc", background: "#fff", borderRadius: "4px", cursor: "pointer" }}
                  onClick={() => { setSelectedReport(r); setModalType("REJECT"); setProcessData({processReason: ""}); }}
                >
                  허위 기각
                </button>
                <button 
                  className={styles.resolveBtn} 
                  style={{ padding: "6px 12px", border: "1px solid #10B981", color: "#10B981", background: "#fff", borderRadius: "4px", cursor: "pointer" }}
                  onClick={() => { setSelectedReport(r); setModalType("RESOLVE"); setProcessData({processReason: "이미 처리된 이슈입니다."}); }}
                >
                  완료 처리
                </button>
                <button 
                  ...
                  onClick={() => { setSelectedReport(r); setModalType("APPROVE"); setProcessData({duration: "DAYS_3", processReason: ""}); }}
                >
                  정지 처리
                </button>
              </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <div className={styles.modalHeader}>
              {modalType === "APPROVE" && `🚨 ${selectedReport.reportedUser?.nickname}님 정지 처리`}
            ...
            {modalType === "APPROVE" && (
              <>
                ...
                <select className={styles.selectField} value={processData.duration} onChange={(e) => setProcessData({...processData, duration: e.target.value})}>
            ...
            <textarea 
              ...
              style={{ width: "100%", padding: "8px", boxSizing: "border-box", marginTop: "4px", minHeight: "80px" }}
