  ...
  const fetchDayData = async (pageNum, day) => {
    ...
    try {
      ...
      const response = await api.get(
        `/api/contents/?type=웹소설&page=${pageNum}&size=10${dayParam}`,
      );
      ...
      console.error("요일별 소설 불러오기 실패:", error);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={styles.heroSection}>
        {HERO_ITEMS.map((item, i) => (
          <img
            ...
            src={item.gif}
            alt=""
            className={`${styles.heroBg} ${i === heroIndex ? styles.heroBgActive : ""}`}
          />
        ))}
          ...
          <div className={styles.heroCard}>
            {HERO_ITEMS.map((item, i) => (
              <img
                ...
                src={item.gif}
                alt={`hero-${i}`}
                className={`${styles.heroCardImg} ${i === heroIndex ? styles.heroCardImgActive : ""}`}
              />
            ))}
          </div>
        ...
        {activeDay === "전체" ? (
          ...
          <div>
            ...
            <div ref={observerRef} className={styles.observer}>
              ...
              {!hasNext && !isLoading && dayContents.length > 0 && (
                <span className={styles.observerText}>
                  모든 작품을 다 보셨습니다.
                </span>
              )}
